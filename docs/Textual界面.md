...
## 八、当前已经有的交互（2026-09-15）

启动后可用：

| 输入 | 行为 |
|------|------|
| `/` | 弹出模糊补全（命令 + Skill），匹配字蓝色 |
| `/su` | 命中 `using-superpowers`、`visualize` 等，上下键选择 |
| `/using-superpowers 任务` | 激活该 Skill，正文进系统提示词 |
| `/skills` | 可滚动列表；输入框可再搜；`/skills sync` 再导入主目录 Skill |
| `/model` | 模型选择弹窗 |
| `/setting` | 昵称 |
| `/quit` | 退出 |

长工具（如 `npx`）期间标题栏和聊天区会转圈显示「正在执行 … Ns」，工具结束后显示「等待模型继续」，出字后收掉。

用户气泡里 `/name`、`$name` 标蓝，其余正文正常颜色。
